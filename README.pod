...
Note that, because the project is moving so much these days, the fact that a version worked in any of the above experiments does not imply that it continues to work. On the other hand, the test suite is already quite extensive and hiccups should be rather exceptional. Once it has been better tested with third party equipment it will be released to CPAN.

=head2 TODO

The distribution could be improved by the following items:

=over

=item 1. Add logging support to the RTU server -- DONE!
